...
def game_loop():
    ...
    if flag:
        ...
        over_lavel.place(x=Width / 3, y=Height / 2 , bg=None)

        # Get the player's name through the input box
        Label(root, text="Please enter your name：").grid(row=0)
        #v = StringVar()
        # Entry for input
        e = Entry(root)
...
def check_begin():
    ...
    def start(event):
        global begin
        if event.keycode == 36: # press Enter to begin
            begin = True
...
def introduction():
    ...
    Label(intro, text="Press 'ESC' to stop and resume").grid(row=1, column=0, padx=10, pady=5)
    Label(intro, text="Press 'c' to cut the length of the snake").grid(row=2, column=0, padx=10, pady=5)
    Label(intro, text="Press 'b' to show the code").grid(row=3, column=0, padx=10, pady=5)
    Label(intro, text="Attention: load the game before press 'Enter' or start a new game!").grid(row=4, column=0, padx=10, pady=5)
